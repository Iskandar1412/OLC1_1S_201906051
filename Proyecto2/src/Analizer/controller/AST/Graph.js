...
node [shape=circle style=filled color=\"#6F6C6B\" fontname= \"Ubuntu\" fontsize=\"14\"];
edge[color=\"#B22222\" penwidth=\"1.5\"];`
    ...
    graph_Instruccion(_up, _instruccion) {
        ...
        else if (_instruccion.tipo === TIPO_INSTRUCCION.EXEC) {
            var child = 'Node' + this.count;
            this.count++;
            this.grafo += child + '[label = \"EXEC\"];\n';
            this.grafo += _up + '->' + child + ';\n';
            this.graph_Llamada(child, _instruccion);
        }
    }
